...
def check_node():
    ...
    if max_no != 0: #自分より長いブロックがあったのならば
        r = requests.get("http://"+node+"/get_block?no="+str(len(block_chain)-1)) #自分の持ってる最新のブロックと相手の持っている同じ番号のブロックを取得
        if r.status_code == 200:
            json_data = r.json()
            if json_data['hash'] == block_chain[-1]['hash']: #ハッシュ値が一致していることを確認する。
                for i in range(len(block_chain), max_no+1): #以降ブロックを繋いでいく(本当はhashとprev_hashがちゃんと繋がっているか検証したほうが良い。
                    r = requests.get("http://" + node + "/get_block?no="+str(i))
                    if r.status_code == 200:
                        json_data = r.json()
